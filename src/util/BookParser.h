...
namespace mislib{
    ...
    static Book parseToBook(const char* line) {
        ...
        detail::copyField(book.title,  line, commaTable[0] + 1, commaTable[1], sizeof(book.title));
        detail::copyField(book.author, line, commaTable[1] + 1, commaTable[2], sizeof(book.author));
        detail::copyField(book.genre,  line, commaTable[2] + 1, commaTable[3], sizeof(book.genre));
